...
function touchPipe(pipe) {
  ...
  if (birdyRight > pipeLeft && birdyLeft < pipeRight) {
    if (birdyDown > pipeDown) {
      if (pipe.children[1].style.backgroundColor === 'green') {
        pipe.children[1].style.backgroundColor = 'red';
        hit--
        alert("il vous reste " + hit + " pv")
      }
    }

    if (birdyUp < pipeUp) {
      if (pipe.children[0].style.backgroundColor === 'green') {
...
function apparitionDesCoeurs(){
  let coeurs = document.getElementById('coeur')
  let coeurWidth = coeurs.offsetWidth

  var left = coeurs.offsetLeft;
  coeurs.style.left = left - 10 + 'px';

  if (left < -coeurWidth) {
    coeurs.style.left = 100 + '%';
